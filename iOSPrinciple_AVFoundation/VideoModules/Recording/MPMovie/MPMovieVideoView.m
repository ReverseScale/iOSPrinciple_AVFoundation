...
@interface MPMovieVideoView ()<MPModelDelegate>
...
@property (nonatomic, strong, readwrite) MPMovieModel *mpModel;

@end

@implementation MPMovieVideoView
...
#pragma mark - view
- (void)BuildUIWithType:(MPVideoViewType)type {
    self.mpModel = [[MPMovieModel alloc] initWithFMVideoViewType:type superView:self];
...
- (void)turnCameraAction {
    [self.mpModel turnCameraAction];
}

- (void)flashAction {
    [self.mpModel switchflash];
}

- (void)startRecord {
    if (self.mpModel.recordState == FMRecordStateInit) {
        [self.mpModel startRecord];
    } else if (self.mpModel.recordState == FMRecordStateRecording) {
        [self.mpModel stopRecord];
    } else if (self.mpModel.recordState == FMRecordStatePause) { 
    }
}
...
- (void)updateRecordState:(FMRecordState)recordState {
    if (recordState == FMRecordStateInit) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(recordFinishWithvideoUrl:)]) {
            [self.delegate recordFinishWithvideoUrl:self.mpModel.videoUrl];
        }
    }
}
